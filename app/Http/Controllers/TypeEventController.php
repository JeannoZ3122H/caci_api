...
class TypeEventController extends Controller
{
    ...
    public function index(){
        try {
            $type_events = TypeEventModel::OrderBy('item_order', 'asc')->groupBy('id', 'item_order')->get();
            ...
            if(empty($request->type_event)):
                return response()->json(
                    [
                        'code' => 302,
                        'status' => 'Erreur',
                        'message' => MessageService::code302('Type évènement')
                    ]
                );
            endif;
    ...
    public function change_item_order(Request $request, $slg){
        try {
            ...
            $resp = TypeEventModel::where('slug', '=', $slg)
