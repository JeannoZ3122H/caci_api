...
// ======= =========== AGENDAS =========== ======= //
    ...
    function cleanDateString($dateString) {
        // Supprimer la partie entre parenthèses (Double timezone specification)
    ...
    public function store_agenda(Request $request){
        try {
            ...
            $cleaned_start_date = $this->cleanDateString($request->date_start_event);
            $cleaned_end_date = $this->cleanDateString($request->date_end_event);

            $add = new AgendasModels();
            $add->author_id = $request->author_id;
            $add->type_event_id = $request->type_event_id;
            $add->google_meet_url = $request->google_meet_url;
            $add->title_event = $request->title_event;
            $add->description_event = $request->description_event;
            $add->date_start_event = Carbon::parse($cleaned_start_date)->format('Y-m-d');
            $add->date_end_event = Carbon::parse($cleaned_end_date)->format('Y-m-d');
            $add->hours_start_event = $request->hours_start_event;
            $add->hours_end_event = $request->hours_end_event;
        ...
        try {
            if(isset($slg)){
                ...
                if($data){
                    ...
                    $data->google_meet_url = $request->google_meet_url;
                    $data->title_event = $request->title_event;
                    $data->description_event = $request->description_event;
                    $data->date_start_event = Carbon::parse($request->date_start_event)->format('Y-m-d');
                    $data->date_end_event = Carbon::parse($request->date_end_event)->format('Y-m-d');
                    $data->hours_start_event = $request->hours_start_event;
                    $data->hours_end_event = $request->hours_end_event;
