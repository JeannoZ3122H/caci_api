...
// ======= =========== LIENS UTILES DETAILS =========== ======= //
    public function index_lien_utile(){
        try {
            $list = LiensUtilesModels::all();
            if($list != null){return $list;}
            else{return $list = [];}
        } catch (\Throwable $e) {
            return response()->json(
    ...
    public function store_lien_utile(Request $request){
        try {
            if(empty($request->author_id)):
                return response()->json(
                    [
            ...
            if(empty($request->illustration_url)):
                return response()->json(
                    [
                        'code' => 302,
                        'status' => 'Erreur',
                        'message' => MessageService::code302('vidéo')
                    ]
                );
            endif;

            $add = new LiensUtilesModels();
            $add->author_id = $request->author_id;
            $add->sub_title = $request->sub_title;
            $add->title = $request->title;
            $add->description = $request->description;
            $add->global_link = $request->global_link;
            $add->illustration_url = $request->illustration_url;
            $add->slug = SlgGenrateService::slgGenerate();
            // (object)
            // return $add;
            if($add->save()){
    ...
    public function update_lien_utile(Request $request, $slg){
        try {
            if(isset($slg)){
                if(empty($request->author_id)):
                    return response()->json(
                ...
                $data = LiensUtilesModels::where('slug', '=', $slg)->first();
                if($data){
                    // return $data;
                    $data->author_id = $request->author_id;
                    ...
                    $data->illustration_url = $request->illustration_url;
                    if($data->save()){
                        return response()->json(
                            [
                                'status' => 'Succès',
    ...
    public function delete_lien_utile($slg){
        try {
            if(!isset($slg)):
                return response()->json(
                    [
            ...
            $resp = LiensUtilesModels::where('slug', '=', $slg)->delete();
            if($resp){
                return response()->json(
                    [
                        'code' => 100,
