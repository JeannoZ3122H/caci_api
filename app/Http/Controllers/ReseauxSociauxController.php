...
class ReseauxSociauxController extends Controller
{
            ...
            if(empty($request->link)):
                return response()->json(
                    [
                        'code' => 302,
                        'status' => 'Erreur',
                        'message' => MessageService::code302('Lien du réseau social')
                    ]
                );
            endif;
            if(empty($request->reseau_social)):
                return response()->json(
                    [
                        'code' => 302,
                        'status' => 'Erreur',
                        'message' => MessageService::code302('Réseau social')
                    ]
                );
            endif;

            $add = new ReseauxSociauxModel();
            $add->author_id = $request->author_id;
            $add->reseau_social = $request->reseau_social;
    ...
    public function update(Request $request, $slg){
        try {
            if(isset($slg)){
                ...
                $data = ReseauxSociauxModel::Where('slug', '=', $slg)->first();
                if($data){
                    $data->author_id = $request->author_id;
                    $data->reseau_social = $request->reseau_social;
    ...
    public function delete($slg){
        try {
            ...
            $resp = ReseauxSociauxModel::where('slug', '=', $slg)->delete();
            if($resp){
                return response()->json(
                    [
                        'code' => 100,
    ...
    public function change_item_order(Request $request, $slg){
        try {
            ...
            $resp = ReseauxSociauxModel::where('slug', '=', $slg)
